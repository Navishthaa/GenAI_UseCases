...
def write_file(file_path: str, content: str) -> str:
    ...
    try:
        ...
        output_path = (EXAMPLE_DIR / file_path).resolve()
        output_path.relative_to(EXAMPLE_DIR.resolve())
        output_path.parent.mkdir(parents=True, exist_ok=True)
        output_path.write_text(content, encoding="utf-8")
        print(f"File successfully written to: {output_path}")
        return f"File written to {output_path}"
    except ValueError:
        return "Error: file_path must stay inside the project directory"
    except Exception as e:
        return f"Error: {e}"
...
def generate_social_image(prompt: str, platform: str, slug: str) -> str:
    ...
    try:
        ...
        print(f"Generated image response: {response}")
...
def create_content_writer():
    ...
    return create_deep_agent(
        model = 'gpt-4o-mini',
        memory=[str(EXAMPLE_DIR / "AGENTS.md")],   # Loaded by MemoryMiddleware
        skills=[str(EXAMPLE_DIR / "skills")],      # Loaded by SkillsMiddleware
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        task = "Write a linkedIn post on topic A2A in agentic AI "
